...
The only instruction has four parameters: a pattern, a template, the name of a function to call on the filled template, and a list of next instructions to execute if the match was succesful. Here is an incomplete "sum" function:
```
"sum" {
    ( "1" . "2" ) -> "3";
    ( "2" . "1" ) -> "sum": ("1" . "2");
...
The `swidy` executable includes a debugger and a LSP. The main command, "run", takes a .swd file with a list of functions, and calls its 'main' function with the rest of cli arguments in a list. The "debug" command takes a .swd file with the S-expression representation of the debugger state.
